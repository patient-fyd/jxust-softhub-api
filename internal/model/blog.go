...
type BlogListInput struct {
	...
	IsRecommend int    `json:"isRecommend" d:"0"` // 是否推荐: 0-全部，1-仅推荐
}
...
type BlogUpdateInput struct {
	BlogId     uint   `json:"blogId" v:"required#博客ID不能为空"`                   // 博客ID
	Title      string `json:"title" v:"required#博客标题不能为空"`                    // 博客标题
	Content    string `json:"content" v:"required#博客内容不能为空"`                  // 博客内容
	Summary    string `json:"summary"`                                        // 博客摘要
	Category   string `json:"category" v:"required#博客分类不能为空"`                 // 博客分类
...
type BlogRecommendInput struct {
	BlogId      uint `json:"blogId" v:"required#博客ID不能为空"`                        // 博客ID
	IsRecommend int  `json:"isRecommend" v:"required|in:0,1#是否推荐不能为空|是否推荐只能为0,1"` // 是否推荐: 0-否，1-是
}
...
type BlogCommentItem struct {
	...
	LikeCount  uint        `json:"likeCount"`  // 点赞数
	ReplyCount int         `json:"replyCount"` // 回复数
	CreateTime *gtime.Time `json:"createTime"` // 创建时间
	// 如果有父评论，则包含父评论信息
	ParentUserId   uint   `json:"parentUserId,omitempty"`   // 父评论用户ID
	ParentUserName string `json:"parentUserName,omitempty"` // 父评论用户昵称
}
...
type BlogCommentCreateInput struct {
	...
	ParentId uint   `json:"parentId"`                      // 父评论ID，可选
}
...
type BlogCommentDeleteInput struct {
	CommentId uint `json:"commentId" v:"required#评论ID不能为空"` // 评论ID
}
